...
def parse_response(response):
    """
    Parses the assistant's response and structures it into slides with:
    - Title
    - Content
    - Keywords for image searches
    """
    slides_content = []
    # Split the response into individual slides using the '---' separator
    slides = re.split(r'\n---\n', response.strip())

    slide_pattern = re.compile(
        r"Slide\s+\d+:\s*Title:\s*(?P<title>.*?)\n"
    ...
    for slide in slides:
        slide = slide.strip()
        match = slide_pattern.match(slide)
        if match:
            title = match.group('title').strip()
            content = match.group('content').strip()
            keywords = match.group('keywords').strip()

            # Remove any trailing '**' from the title
            ...
            print(f"Warning: Slide parsing failed for slide:\n{slide}\n")
...
def create_ppt(slides_content, template_choice, presentation_title, presenter_name, insert_image):
        ...
        if template_choice == 'geometric_template':
            ...
            for placeholder in slide.placeholders:
                if placeholder.placeholder_format.type == 1:
                    ...
                    placeholder.text_frame.paragraphs[0].font.size = Pt(22)
    ...
    prs.save(os.path.join(temp_dir, 'generated_presentation.pptx'))
